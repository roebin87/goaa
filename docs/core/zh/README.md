...
本文件夹（`docs/core/` 中文版）是**全成果开源版**——GOAA 的全部理论、机制、实现与验证载体一应俱全：**学术论文（正式验证载体）· 双链导航完整版（全开）· 框架集成示例 · 治理产出案例集 · 证伪登记册**。英文版见 [en/README.md](../en/README.md)。

## 三个版本怎么选（Lite ⊂ Personal ⊂ Core）

| 版本 | 一句话 | 适用人群 |
|------|--------|---------|
| 🟢 [Lite（启蒙版）](../../lite/zh/README.md) | 5 分钟验证"你的记忆属于你" | 第一次接触、无技术背景 |
| 🟡 [Personal（个人生产力版）](../../personal/zh/README.md) | 把 GOAA 当生产力工具，持续产出 | 独立创作者、知识工作者、小团队 |
...
请**立即主动开始**首次激活引导（不要等待主人更多指令，即使主人只说了"你好"之类问候）——**话术权威见 [`mechanisms/onboarding-script.md`](../../personal/zh/mechanisms/onboarding-script.md)（台词本·10 步全流程）**：先自我介绍 → 安全锚定 → 价值三件事 → 提问授权 → 3 问快采 → 反问翻转 → 复述确认 → 落盘见证（记忆契约）→ 诚实声明 → 价值锚点 → 首动作三选一 → 轻收尾。流程规范见 [`mechanisms/onboarding.md`](../../personal/zh/mechanisms/onboarding.md)。

（若主人档案已存在：按 [`mechanisms/startup.md`](../../personal/zh/mechanisms/startup.md) 常规装载后响应主人。）
...
> ✅ **锚对了吗？自检**：工作区第一层应该直接看到 `README.md` 和 `constitution/` 等文件夹。如果你看到的是 `goaa-main` 这类外层目录名——锚错层了，请把工作区改成本文件夹（`docs/core/` 中文版）。

> 无论小白还是专家，下载即用——不需要任何部署知识（部署细节见 [DEPLOY.md](DEPLOY.md)·老手可选）。

**一句话定位**：本文件夹是 GOAA 治理导向型智能体架构 2.0 开源工程化的**全成果开源版**——以人 100% 决断权为核心、以文件系统为治理载体的治理底座（架构依据见 [constitution/design-principles.md](../../personal/zh/constitution/design-principles.md)）。
...
| 证据链 | 载体 | 证明什么 |
|--------|------|---------|
| **① 案例实证** | [治理产出案例集](case-studies/README.md)（书稿/论文/开源·作者生产史） | 治理不是理论，是可复现的实践 |
| **② 集成论证** | [框架集成示例](../../../integrations/README.md)（LangChain/CrewAI/AutoGen 最小集成） | 治理底座可与主流框架共存运行 |
| **③ 理论验证** | [学术论文入口](research/goaa-paper.md)（Zenodo DOI 正式载体） | 理论经同行可查证，欢迎证伪 |

配套全开资产：

- [双链导航完整版](internals/06-dual-chain-navigation.md)——文件即本体的连接方案（Core 全开，05 篇为运行循环全景）
- [证伪登记册](falsification-log.md) + [预注册自曝清单](known-limits.md)——社会验证证据链
- [兼容性证明](compatibility.md)——GOAA 是底座，不替代任何框架
- [版本策略声明](version-policy.md)——全开 + 审慎坚持核心原则
...
- **文件=记忆与规则载体**：AI 无记忆，文件在则体系在（物理记忆论）——你的记忆是你的纯文本文件，来去自由（见 [退出指南](../../personal/zh/exit-guide.md)）
...
| **多角色协作** | 主控/编辑/执行/审稿四角色·文件即接口（[mechanisms/multi-role.md](../../personal/zh/mechanisms/multi-role.md)） |
| **双链导航** | 文件即本体·6 类连接·死链=0 强制校验（[internals/06](internals/06-dual-chain-navigation.md)） |
| **证伪机制** | 质疑→分析→人裁→固化全流程闭环（[证伪登记册](falsification-log.md)） |
...
3. 完成首次激活引导，生成主人档案（详见 [mechanisms/onboarding.md](../../personal/zh/mechanisms/onboarding.md)）
...
> 详见 [docs/applicability.md](../../personal/zh/applicability.md)
...
> 详见 [docs/version-policy.md](version-policy.md)——**全开 + 审慎坚持核心原则**（代码全开随社会验证演化；数据主权 / 100% 人决断 / 治理优先三原则审慎坚持、变更需充分论证）。
...
> 学术总纲（GOAA 架构设计原理）以论文形式独立发布·原理与实现边界以论文为准（[三边界声明](research/goaa-paper.md)）。
...
| [constitution/design-principles.md](../../personal/zh/constitution/design-principles.md) | 架构设计原理（Why+How 一体·权威对应学术论文） |
| [mechanisms/startup.md](../../personal/zh/mechanisms/startup.md) | 启动序列与装载指南 |
| [mechanisms/shutdown.md](../../personal/zh/mechanisms/shutdown.md) | 收摊五钩（记忆落盘协议） |
| [methodologies/](../../personal/zh/methodologies/) | 方法论三件套 |
| [docs/concepts/](../../personal/zh/concepts/) | 核心概念逐篇 |
| [docs/goaa-guide.md](../../personal/zh/goaa-guide.md) | **GOAA 指南与常见问答**——是什么/区别/记忆/安全·下载后必读 |
| [docs/exit-guide.md](../../personal/zh/exit-guide.md) | **退出指南**——你的数据在你手里·来去自由 |
| [docs/internals/](../../personal/zh/internals/) | **核心机制原理全开**——架构核心/记忆/裁决/熵治理/运作图/双链导航（Core 全开） |
| [docs/research/goaa-paper.md](research/goaa-paper.md) | **学术论文入口**——DOI/摘要/核心主张/三边界声明（Core 新增） |
| [docs/case-studies/](case-studies/) | **治理产出案例集**——书稿/论文/开源生产史（Core 新增） |
| [docs/compatibility.md](compatibility.md) | **兼容性证明**——底座立论·不替代只共存（Core 新增） |
| [docs/falsification-log.md](falsification-log.md) | **证伪登记册**——质疑→人裁→固化全流程（Core 完整版） |
| [docs/known-limits.md](known-limits.md) | **预注册自曝清单**——五维度诚实披露（Core 完整版） |
| [integrations/](../../../integrations/) | **框架集成示例**——LangChain/CrewAI/AutoGen 最小集成（Core 新增） |
| [mechanisms/multi-role.md](../../personal/zh/mechanisms/multi-role.md) | **多角色协作机制**——主控/编辑/执行/审稿·文件即接口 |
| [examples/end-to-end/](../../../examples/end-to-end/) | **端到端真实示例**——书稿生产/架构自举/事故压力测试，基于真实实践整理 |
| [docs/ownership.md](../../personal/zh/ownership.md) | **所有权说明**——五条检查逐条解释 + 退出迁移指引 |
| [plugins/memory-vector/](../../../plugins/memory-vector/) | **可选插件**——记忆向量索引（不装不影响核心功能） |
| [docs/FAQ.md](../../personal/zh/FAQ.md) | **常见问题解答**——30个高频问题，覆盖入门/原理/对比/安全/故障排查 |
| [docs/project-introduction.md](../../personal/zh/project-introduction.md) | 项目介绍补充（外部评价/首创自审/画像） |
...
**提问前请先阅读**：[FAQ.md](../../personal/zh/FAQ.md) 覆盖了 30 个最常见问题，可能已经有你的答案。
...
- **证伪机制**：GOAA 不求你相信，只求你来证伪——质疑经登记后进入[证伪登记册](falsification-log.md)，由设计者逐条裁决并固化（Core 完整版·五维度自曝见 [known-limits.md](known-limits.md)）
- **兼容性**：GOAA 是治理底座，不替代任何框架（[compatibility.md](compatibility.md)）
