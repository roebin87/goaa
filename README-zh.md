...
| 轨 | 编号 | 含义 |
|---|---|---|
| **架构版本** | 1.0 | 核心稳定版（宪法/规则/核心机制）——大版本保持稳定 |
| **发行版本** | v0.1.x | 仓库发行迭代号（bug 修复/体验优化 = v0.1.x · 新增外围件 = v0.2.x · 正式稳定 = v1.0） |
| **论文版本** | V2.x | 学术线（独立迭代） |

> **发行版号 ≠ 架构版本**：`v0.1.0` 只是仓库发行号，不代表架构版本变化。权威源见 [VERSION](VERSION) · 策略细则见[版本策略](docs/core/zh/version-policy.md)。
