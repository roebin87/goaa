...
MEMORY_PATHS = [
    "docs/lite/en/templates/memory",
    "docs/lite/en/templates/identity",
    "docs/lite/en/identity",
]
...
def scan_text_files(root: Path):
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for name in filenames:
            p = Path(dirpath) / name
            if p.suffix.lower() in (".py", ".pyc", ".exe", ".bin", ".png", ".jpg", ".jpeg", ".gif", ".ico", ".svg"):
                continue
            files.append(p)
    return files
...
META_FILES = {"readme.md", "readme-zh.md", "changelog.md", "changelog-zh.md", "citation.cff", "license"}
...
def check_no_remote_refs():
    ...
    for p in scan_text_files(ROOT):
        if "verify-ownership.py" in p.name or p.suffix == ".py" or ".github" in p.parts:
